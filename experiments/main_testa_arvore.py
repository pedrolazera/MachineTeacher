...
qtd_runs_per_test = 10

# Learner
get_learner = machine_teacher.Learners.RandomForestLearner
learner_params = {
	"random_state": 0,
	"n_estimators": 1,
	"max_depth": 10
...
# Teacher
get_teacher = machine_teacher.Teachers.SingleBatchTeacher
teacher_params = {}
...
datasets_names = [
	...
	("car.csv", False),
	("ClaveVectors_Firm_Teacher_Model.csv", False),
...
for (dataset_name, is_numeric) in datasets_names:
	for rel_size in dataset_rel_sizes:
		...
		for __ in range(qtd_runs_per_test):
			L = get_learner(**learner_params)
			T = get_teacher(**teacher_params)
