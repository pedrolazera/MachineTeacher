...
class DynamicPercWrongTeacher(Teacher):
	...
	def start(self, X, y, time_left: float):
		...
		self.f_choice = np.random.RandomState(self.seed).choice
	...
	def get_new_examples(self, test_ids, test_labels, time_left: float):
		...
		correct_test_labels = self.y[test_ids[self.S_current_size:]]
		wrong_ids = test_ids[self.S_current_size:] [test_labels[self.S_current_size:] != correct_test_labels]
		correct_ids = test_ids[self.S_current_size:] [test_labels[self.S_current_size:] == correct_test_labels]
		new_ids = np.append(test_ids[:self.S_current_size], wrong_ids)
		new_ids = np.append(new_ids, correct_ids[:self.batch_size - len(new_ids)])
	...
	def get_new_test_ids(self, test_ids,
		...
		if self.state_new_ids == self._STATE_SEND_NEW_IDS:
			if self.S_current_size >= self.m/2.0:
				self.state_new_ids = self._STATE_SEND_EMPTY_NEW_IDS
				return self.shuffled_ids

			random_set_ids = self.f_choice(
				self.m-self.S_current_size - self.qtd_untrained_tested_examples, 
				self.S_current_size,
				replace=False)
			random_set_ids += (self.S_current_size + self.qtd_untrained_tested_examples)

			random_set_V = np.copy(self.shuffled_ids[random_set_ids])
			self.shuffled_ids[random_set_ids] = np.copy(self.shuffled_ids[:self.S_current_size])
			self.shuffled_ids[:self.S_current_size] = np.copy(random_set_V)

			self.state_new_ids = self._STATE_CHOOSE_BATCH_SIZE_NEW_IDS
			
			print(self.S_current_size, self.batch_size, np.unique(self.shuffled_ids).size)
			assert np.unique(self.shuffled_ids).size == self.m
			assert len(test_ids) == 0
			return self.shuffled_ids[:self.S_current_size]
		...
		if self.state_new_ids == self._STATE_CHOOSE_BATCH_SIZE_NEW_IDS:
			...
			if error < 0.5:
				q = ( (error/(1-error))**(2/3) ) / ( 1 + (error/(1-error))**(2/3) )
			else:
				q = error
			
			increment = int((q*self.batch_size)/error)
			increment -= self.S_current_size
			
			if self.S_current_size < self.m/4.0:				
				increment = min(self.m-3*self.S_current_size, increment)
			else:
				increment = min(self.m-self.S_current_size, increment)

			print(self.batch_size, increment, 'e: ', error, ', q: ', q, '\n\n')

			assert increment >= 0

			random_set_ids = self.f_choice(
				self.m-self.S_current_size, # m - tamanho do conjunto de validacao
				increment,
				replace=False)			
			random_set_ids += self.S_current_size
			
			for i in range(increment):
				i1 = self.S_current_size+i
				i2 = random_set_ids[i]
				self.shuffled_ids[i1], self.shuffled_ids[i2] = self.shuffled_ids[i2], self.shuffled_ids[i1]

			_new_index_U = [i for i in range(self.S_current_size, self.S_current_size+increment)]
			self.state_new_ids = self._STATE_SEND_EMPTY_NEW_IDS
			assert len(np.unique(np.append(self.shuffled_ids[_new_index_U], test_ids))) == (len(test_ids)+len(self.shuffled_ids[_new_index_U]))
			return self.shuffled_ids[_new_index_U]
...
###########################################
# treina uma vez, seleciona uma fracao errada e 
# restante aleatorios
